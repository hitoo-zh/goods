        ...
        <%--<div class="portlet-body">
            <div class="form-inline" role="form" id="table-param">
                <div class="form-group form-md-line-input has-success">
                    <input type="text" class="form-control" name="name" value="${artName}" autofocus placeholder="名称">
            ...
            <table data-current-page="${pageInfo.pageNum}" data-page-size="${pageInfo.pageSize}"
                ...
                <c:forEach var="item" items="${pageInfo.list}" varStatus="status">
                    <tr>
                        ...
                        <td>${item.id}</td>
                        <td><a href="javaScript:;" onclick="detail('${item.id}')">${item.artName}</a></td>
                        <td>${item.catName}</td>
                        <td><img src="${item.img}" style="width: 200px;height: 60px"></td>
                        <td>${item.mark}</td>
                        <td>
...
<script>
     ...
     */
    function addModule(){
        tools.loadPage("/article/add", null, function(html){
...
function detail(id) {
    $.post("/article/detail", {"id":id}, function(html){
    ...
    function editModule(id){
        $.post("/article/edit", {"id":id}, function(html){
