            ...
            <table data-current-page="${pageInfo.pageNum}" data-page-size="${pageInfo.pageSize}"
                ...
                <c:forEach var="item" items="${pageInfo.list}" varStatus="status">
                    <tr>
                        ...
                        <td>${item.createManagerId}</td>
                        <td>${item.mark}</td>
                        <td>
                            <div class="btn-group btn-group-xs btn-group-solid ">
                                <button data-id="${item.id}" class="btn btn-danger btn-module-delete" onclick="deleteById('${item.id}')">删除</button>
                            </div>
                        </td>
                    </tr>
                </c:forEach>
   ...
   $(document).ready(function () {
    ...
    function deleteById(id) {
        layer.confirm("确定删除么？", function () {
            tools.post("/fkucun/delete", {"id": id}, function (data) {
                if (data.success) {
