            ...
            <table data-current-page="${pageInfo.pageNum}" data-page-size="${pageInfo.pageSize}"
                ...
                <c:forEach var="item" items="${pageInfo.list}" varStatus="status">
                    <tr>
                        ...
                        <td>${item.name}</td>
                        <td style="text-align:center;width: 5%;height: 5%;"><img src="${item.img}" style="text-align:center;width: 5%;height: 5%;"/></td>
                        <td>${item.jinPrice}</td>
...
<script>
     ...
     */
    function addModule() {
        tools.loadPage("/fgoods/add", null, function (html) {
    ...
    function editModule(id) {
        $.post("/fgoods/edit", {"id": id}, function (html) {
    ...
    function deleteById(id) {
        layer.confirm("确定删除么？", function () {
            tools.post("/fgoods/delete", {"id": id}, function (data) {
                if (data.success) {
    ...
    function deleteByIds() {
        ...
        layer.confirm("确定删除选中信息么？", function () {
            tools.post("/fgoods/deleteByIds", {"ids": ids.join(",")}, function (data) {
                if (data.success) {
