                ...
                <div class="col-md-3">
                    ...
                    <div class="btn-group">
                        ...
                        <a href="javascript:void(0);" class="btn green btn-module-add "
                           onclick="addModule('${pageInfo.list[0].id}')">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group btn-group-xs btn-group-solid ">
                                <button data-id="${item.id}" class="btn btn-success btn-module-edit"
                                        onclick="ruku('${item.id}')">入库
                                </button>
                                <button data-id="${item.id}" class="btn btn-danger btn-module-delete">删除</button>
                                <button data-id="${item.id}" onclick="xiangqingById('${item.id}')"
                                        class="btn btn-danger" style="background-color:#ab5;">入库详情
                                </button>
                            </div>
                        </td>
                    </tr>
...
<script>
     ...
     * 添加模块
     */
    function addModule(storeId) {
        tools.loadPage("/fkucun/selectGoodsClass", {"storeId": storeId}, function (html) {
     ...
     */
    function ruku(id) {
        $.post("/fkucun/ruku", {"id": id}, function (html) {
     ...
     * @param id 模块名
     */
    function xiangqingById(id) { //fkucun/list
        location.href = "/fkucun/xiangqing?id=" + id + "&&page=1&&size=10";
    }
    ...
    function deleteById(id) {
        layer.confirm("确定删除么？", function () {
            tools.post("/fkucun/delete1", {"id": id}, function (data) {
                if (data.success) {
    ...
    function deleteByIds() {
        ...
        layer.confirm("确定删除选中信息么？", function () {
            tools.post("/fkucun/deleteByIds", {"ids": ids.join(",")}, function (data) {
                if (data.success) {
